// tb_system_spi.sv -- the whole control and signal path as real RTL.
//
// Real PicoRV32, soc_bus, firmware, SICU, LSTM accelerator AND spi_master,
// which drives CS/SCLK/MOSI/MISO pins into a bit-level SPI flash model
// (mode 0, 0x03 READ, 24-bit address, auto-increment). Only the flash chip,
// the I/Q source and the SRAM macros are behavioural.
//
// Built with +define+BAD_FLASH it loads a corrupted image and checks the
// retry path at pin level: the flash must see three separately framed READ
// commands, which proves chip-select really rises between bursts.
//
// Earlier description, still applicable:
...
module tb_system_spi;
  ...
  wire spi_start, spi_hold_cs; wire [7:0] spi_tx; reg [7:0] spi_rx;
  reg spi_done;
  ...
  // ---- REAL SPI master ------------------------------------------------------
  // clk_div 1: SCLK = clk / 4 -- 8.25 MHz at 33 MHz, well inside what any
  // flash part supports for a plain 0x03 READ.
  wire spi_cs_n, spi_sclk, spi_mosi, spi_miso;
  wire spi_busy_w;
  wire [7:0] spi_rx_w; wire spi_done_w;
  spi_master #(.DIV_WIDTH(8), .DATA_WIDTH(8)) u_spi (
    .clk_i(clk), .rst_ni(rst_n), .clk_div_i(8'd1),
    .start_i(spi_start), .hold_cs_i(spi_hold_cs), .tx_data_i(spi_tx),
    .rx_data_o(spi_rx_w), .busy_o(spi_busy_w), .done_o(spi_done_w),
    .cs_no(spi_cs_n), .sclk_o(spi_sclk), .mosi_o(spi_mosi), .miso_i(spi_miso));
  always @(*) begin spi_rx = spi_rx_w; spi_done = spi_done_w; end

  integer n_spi = 0;
  always @(posedge clk) if (spi_start) n_spi = n_spi + 1;

  // ---- bit-level SPI flash: mode 0, 0x03 READ, full 24-bit address ----------
  // Structure follows tb/tb_spi_master.sv; extended from 256 bytes to the full
  // image, and it counts every command so framing can be checked.
  ...
  reg        f_miso;
  integer    n_cmd = 0, n_bad_cmd = 0, n_cs_fall = 0;
  assign spi_miso = spi_cs_n ? 1'bz : f_miso;
  initial begin
    for (k = 0; k < 4096; k = k + 1) flash[k] = 8'hFF;
`ifdef BAD_FLASH
    $readmemh("flash_bad.hex", flash);
`else
    $readmemh("flash.hex", flash);
`endif
  end
  always @(negedge spi_cs_n) begin
    f_bitcnt <= 0; f_state <= 0; f_shift_in <= 0; f_miso <= 1'b0;
    n_cs_fall = n_cs_fall + 1;
  end
  always @(posedge spi_sclk) if (!spi_cs_n) begin
    ...
    if (f_bitcnt == 3'd7) begin
      case (f_state)
        3'd0: begin
          f_cmd <= {f_shift_in[6:0], spi_mosi}; f_state <= 1;
          n_cmd = n_cmd + 1;
          if ({f_shift_in[6:0], spi_mosi} !== 8'h03) n_bad_cmd = n_bad_cmd + 1;
        end
  ...
  always @(negedge spi_sclk) if (!spi_cs_n) begin
    if (f_state == 4) begin
      if (f_bitcnt == 0) begin
        f_shift_out <= flash[f_addr[11:0]];
        f_miso      <= flash[f_addr[11:0]][7];
...
`ifndef BAD_FLASH
  initial begin
    ...
    $display("       flash saw %0d command(s), %0d CS falls, %0d not 0x03", n_cmd, n_cs_fall, n_bad_cmd);
    if (n_cmd != 1 || n_bad_cmd != 0 || f_addr === 24'hx) begin
      $display("FAIL  expected exactly one 0x03 READ"); errors = errors + 1;
    end else $display("pass  one framed 0x03 READ, burst of %0d bytes on the pins", n_spi - 4);
...
`else
  initial begin
    ...
    $display("cycle %0d  weights_fault after %0d SPI bytes", cyc, n_spi);
    $display("       flash saw %0d command(s) over %0d CS falls, %0d not 0x03",
             n_cmd, n_cs_fall, n_bad_cmd);
    if (n_cmd == 3 && n_cs_fall == 3 && n_bad_cmd == 0)
      $display("pass  three separately framed READs -- CS rises between retries");
    else begin $display("FAIL  retry framing"); errors = errors + 1; end
    repeat (50000) @(posedge clk);
    if (weights_ready || sicu_enable) begin $display("FAIL  left the safe state"); errors = errors + 1; end
    else $display("pass  stays not-ready and never starts measuring");
    if (spi_cs_n !== 1'b1) begin $display("FAIL  CS left low after giving up"); errors = errors + 1; end
    else $display("pass  CS released after giving up");
    if (errors == 0) $display("PASS: SPI fault path at pin level");
    else             $display("FAIL: SPI fault path, %0d errors", errors);
    $finish;
  end
`endif
