...
if len(sys.argv) > 1:
    # The real exported image, e.g. rtl/weights/weights.hex
    words = [int(l, 16) for l in open(sys.argv[1]).read().split()][:512]
    MU, INVSD = words[0] & 0xFFFF, words[1] & 0xFFFF
else:
    # Small random weights so the recurrence does not saturate.
    MU, INVSD = 0x0020, 0x0145
    def small(): return random.randint(-96, 96) & 0xFFFF
    words = [(small() << 16) | small() for _ in range(512)]
